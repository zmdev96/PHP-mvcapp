...
class Template
{
  use TemplateHelper;
     ...
     */
    public function __construct($templatePath = null, $compiledPath = null, $mode = 0, $custom_method)
    {
        $this->_registry = $custom_method;

        if ($templatePath === null) {
            $templatePath = \getcwd() . '/views';
        }
        if ($compiledPath === null) {
            $compiledPath = VIEWS_COMPILES_PATH;
        }
    ...
    {
        trigger_error('BladeOne Error ['. $id .']: ' . $text, E_USER_ERROR);
    }
    ...
    public function isExpired($fileName)
    {
        ...
        if (!\file_exists($template)) {
            if ($this->mode == self::MODE_DEBUG) {
                $this->showError("Read file", "View file not found :" . $this->fileName . " on file: $template", true);
            } else {
                $this->showError("Read file", "View file not found :" . $this->fileName, true);
            }
        }
